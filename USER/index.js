...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const helpModalClose = $('helpModalClose');
    ...
    adminGuideLink.addEventListener('click', (e) => {
      e.preventDefault();
      // populate modal content with the user guide content (HTML tags inserted)
      const modalBody = $('helpModalBody');
      if (modalBody) {
        modalBody.innerHTML = `
          <h3>NC Assessment Dashboard — User Guide</h3>
          <p>This guide is for teachers who may upload files and preview saved files.</p>

          <h4>Uploading</h4>
          <ol>
            <li>Use the Upload button and choose an .xlsx/.xls/.csv file.</li>
            <li>Files are saved locally (My Files) and will also be sent to the server if available.</li>
          </ol>

          <h4>Viewing Saved Files</h4>
          <p>Click <em>My Files</em> to preview or load saved files.</p>

          <h4>Troubleshooting</h4>
          <ul>
            <li><strong>Problem:</strong> File won't upload.<br/><strong>Cause:</strong> Backend not running, wrong file type, or network issue.<br/><strong>Fix:</strong> Ask an administrator to upload, or ensure the backend is running at <code>http://127.0.0.1:5000</code> and use .xlsx/.xls/.csv.</li>

            <li><strong>Problem:</strong> Charts show no data.<br/><strong>Cause:</strong> No valid rows or wrong date format (Date Assessed should be <code>YYYY-MM-DD</code>).<br/><strong>Fix:</strong> Ask admin to check the file and re-upload using the provided template.</li>

            <li><strong>Problem:</strong> Server error (500) or Unauthorized (401/403).<br/><strong>Cause:</strong> Backend error or expired/missing token.<br/><strong>Fix:</strong> Contact an administrator and share the error details and time.</li>

            <li><strong>Problem:</strong> Some rows not imported or validation warnings.<br/><strong>Cause:</strong> Missing/mismatched headers, duplicates, or invalid values.<br/><strong>Fix:</strong> Ask admin to check headers and data, fix any issues, and re-import.</li>

            <li><strong>Tip:</strong> If you're still stuck, take a screenshot and send it to the administrator with the file name and upload time.</li>
          </ul>
        `;
  ...
  /* My Files UI removed per user request. */

  // File storage helpers remain available if needed for admin workflows.
    ...
    Object.keys(yearGroups).sort((a,b)=>b-a).forEach(year => {
      ...
      const yearSection = document.createElement('div'); yearSection.className = 'year-section';
        ...
        const rowsHtml = yearRows.map((rLocal) => {
          ...
          return `<tr ${idAttr} data-index="${globalIdx}"><td>${rLocal.name||''}</td><td>${rLocal.gender||''}</td><td>${rLocal.qualification||''}</td><td>${rLocal.dateAssessed||''}</td><td>${rLocal.assessmentCenter||''}</td><td class="pass">${rLocal.result||''}</td><td>${rLocal.ncNo||''}</td><td>${rLocal.school||''}</td></tr>`;
        }).join('');
        yearSection.innerHTML = `<h3>${year}</h3><table><thead><tr><th>Name</th><th>Gender</th><th>Qualification</th><th>Date Assessed</th><th>Assessment Center</th><th>Result</th><th>NC Number</th><th>School</th></tr></thead><tbody>${rowsHtml}</tbody></table>`;
...
/* [Event delegation] Handle general click events (edit/delete buttons removed) */
